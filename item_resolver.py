...
def find_item_text(item_id, text_data, config, is_description=False):
    ...
    for id_prefix, name_prefix in prefixes.items():
        if item_id_str.startswith(id_prefix):
            key = ""
            if is_description:
                # Constructs description key, e.g., "item_food_d_10201004"
                desc_prefix = name_prefix.rstrip('_') + "_d_"
                key = f"{desc_prefix}{item_id_str}"
            else:
                # Constructs name key, e.g., "item_food_10201004"
    ...
    try:
        ...
        for filename, id_key in extra_data_config.items():
            ...
            if data.get(file_key) and data[file_key].get('list'):
                extra_data_maps[filename] = {item[id_key]: item for item in data[file_key]['list']}

        # --- Combine master_item_common with items from other master files ---
        all_items_to_process = data['master_item_common'].get('list', [])
        existing_item_ids = {item['itemId'] for item in all_items_to_process}

        # Scan master_material for unique items
        ...
        for item in all_items_to_process:
            ...
            resolved_item['itemName_EN'] = find_item_text(item_id, data['master_text_EN'], config)
            resolved_item['itemName_JA'] = find_item_text(item_id, data['master_text_JA'], config)
            resolved_item['itemDescription_EN'] = find_item_text(item_id, data['master_text_EN'], config, is_description=True)
            resolved_item['itemDescription_JA'] = find_item_text(item_id, data['master_text_JA'], config, is_description=True)
            ...
            if category_id in item_category_map:
                ...
                if name_key:
                    category_info['name_EN'] = data['master_text_EN'].get(name_key)
                    category_info['name_JA'] = data['master_text_JA'].get(name_key)
