            ...
            if key in ['drop_rules', 'spawners', 'reaction_items']:
                ...
                for element in sorted(list(set(old_res.keys()) | set(new_res.keys()))):
                    if old_res.get(element) != new_res.get(element):
                        diffs.append(f"  - Element {element} resistance changed from {old_res.get(element, 'N/A')} to {new_res.get(element, 'N/A')}")
                 ...
                 if old_value.get('name_EN') != new_value.get('name_EN'):
                     diffs.append(f"  - Category changed from '{old_value.get('name_EN')}' to '{new_value.get('name_EN')}'")
                ...
                for source_file in set(old_extra.keys()) | set(new_extra.keys()):
                    if old_extra.get(source_file) != new_extra.get(source_file):
                        diffs.append(f"  - Extra data from '{source_file}' has changed.")
...
def main():
    ...
    # NEW: Argument for seeding creature history
    ...
    resolved_enchants_file = os.path.join(args.working_dir, 'resolved_enchants.json')
    resolved_items_file = os.path.join(args.working_dir, 'resolved_items.json')
    resolved_creatures_file = os.path.join(args.working_dir, 'resolved_creatures.json') # NEW
    ...
    historical_enchant_ingredients_file = os.path.join(args.output_dir, 'historical_enchant_ingredients.json')
    historical_enchant_skills_file = os.path.join(args.output_dir, 'historical_enchant_skills.json')
    historical_items_file = os.path.join(args.output_dir, 'historical_items.json')
    historical_creatures_file = os.path.join(args.output_dir, 'historical_creatures.json') # NEW

    lua_recipes_file = os.path.join(args.output_dir, 'historical_recipes.lua')
    lua_droptables_file = os.path.join(args.output_dir, 'historical_droptables.lua')
    lua_enchant_ingredients_file = os.path.join(args.output_dir, 'historical_enchant_ingredients.lua')
    lua_enchant_skills_file = os.path.join(args.output_dir, 'historical_enchant_skills.lua')
    lua_items_file = os.path.join(args.output_dir, 'historical_items.lua')
    lua_creatures_file = os.path.join(args.output_dir, 'historical_creatures.lua') # NEW

    # Add the new creature resolver to the main execution block
    if not run_decryption(args.input_dir, decrypted_dir) or \
       ...
       not run_script('item_resolver.py', decrypted_dir, resolved_items_file) or \
       not run_script('creature_resolver.py', decrypted_dir, resolved_creatures_file): # NEW
        return

    # Add the new historical file to the version check
    version_string = get_version_from_user(
        [historical_recipes_file, historical_droptables_file, historical_enchant_ingredients_file, 
         historical_enchant_skills_file, historical_items_file, historical_creatures_file])
    ...
    else: print("No recipe changes detected.")
    ...
    else: print("No drop table changes detected.")
    ...
    # Process Enchants
    with open(resolved_enchants_file, 'r', encoding='utf-8') as f: resolved_enchants = json.load(f)
    ...
    ing_report_lines = [f"Enchant Ingredient Change Report for version: {version_string}\n"]
    if update_history_file({"enchant_ingredients": resolved_enchants.get("enchant_ingredients", [])}, historical_ingredients, version_string, enchant_ing_id_map, ing_report_lines):
        with open(os.path.join(args.output_dir, f'report_enchant_ingredients_{version_string}.txt'), 'w', encoding='utf-8') as f: f.write('\n'.join(ing_report_lines))
        print("Enchant ingredient report generated.")
    else: print("No enchant ingredient changes detected.")
    with open(historical_enchant_ingredients_file, 'w', encoding='utf-8') as f: json.dump(historical_ingredients, f, indent=4, ensure_ascii=False)
    ...
    skill_report_lines = [f"Enchant Skill Change Report for version: {version_string}\n"]
    if update_history_file({"enchant_skills": resolved_enchants.get("enchant_skills", [])}, historical_skills, version_string, enchant_skill_id_map, skill_report_lines):
        with open(os.path.join(args.output_dir, f'report_enchant_skills_{version_string}.txt'), 'w', encoding='utf-8') as f: f.write('\n'.join(skill_report_lines))
        print("Enchant skill report generated.")
    else: print("No enchant skill changes detected.")
    with open(historical_enchant_skills_file, 'w', encoding='utf-8') as f: json.dump(historical_skills, f, indent=4, ensure_ascii=False)
    ...
    with open(resolved_items_file, 'r', encoding='utf-8') as f: resolved_items = json.load(f)
    ...
    if update_history_file(resolved_items, historical_items, version_string, item_id_map, item_report_lines):
        with open(os.path.join(args.output_dir, f'report_items_{version_string}.txt'), 'w', encoding='utf-8') as f: f.write('\n'.join(item_report_lines))
        print("Item report generated.")
    else: print("No item changes detected.")
    with open(historical_items_file, 'w', encoding='utf-8') as f: json.dump(historical_items, f, indent=4, ensure_ascii=False)
    ...
    historical_creatures = json.load(open(historical_creatures_file, 'r', encoding='utf-8')) if os.path.exists(historical_creatures_file) else (seed_history(
    ...
    with open(resolved_creatures_file, 'r', encoding='utf-8') as f: resolved_creatures = json.load(f)
    creature_report_lines = [f"Creature Change Report for version: {version_string}\n"]
    if update_history_file(resolved_creatures, historical_creatures, version_string, creature_id_map, creature_report_lines):
        with open(os.path.join(args.output_dir, f'report_creatures_{version_string}.txt'), 'w', encoding='utf-8') as f: f.write('\n'.join(creature_report_lines))
        print("Creature report generated.")
    else: print("No creature changes detected.")
    with open(historical_creatures_file, 'w', encoding='utf-8') as f: json.dump(historical_creatures, f, indent=4, ensure_ascii=False)
    write_lua_module(lua_creatures_file, historical_creatures)
