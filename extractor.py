...
def python_to_lua_string(data, indent=0):
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            key_str = json.dumps(key, ensure_ascii=False) if isinstance(key, str) else str(key)
...
def run_decryption(input_dir, decrypted_dir):
    ...
    if not os.path.exists(script_path):
        ...
        return False
    try:
        result = subprocess.run(['python', script_path, input_dir, decrypted_dir], check=True, capture_output=True, text=True, encoding='utf-8')
        print(result.stdout)
        if result.stderr: print("Decryption script stderr:\n", result.stderr)
...
def run_script(script_name, decrypted_dir, output_file):
    ...
    with open(original_script_path, 'r', encoding='utf-8') as f_in, open(temp_script_path, 'w', encoding='utf-8') as f_out:
        for line in f_in:
            if line.strip().startswith("INPUT_DIRECTORY ="): f_out.write(f"    INPUT_DIRECTORY = r'{decrypted_dir}'\n")
            elif line.strip().startswith("OUTPUT_FILE ="): f_out.write(f"    OUTPUT_FILE = r'{output_file}'\n")
            else: f_out.write(line)
    try:
        result = subprocess.run(['python', temp_script_path], check=True, capture_output=True, text=True, encoding='utf-8')
        print(result.stdout)
        if result.stderr: print(f"{script_name} stderr:\n", result.stderr)
...
def generate_diff(old_data, new_data, ignored_keys=None):
    ...
    for key in sorted(list(all_keys - (ignored_keys or set()))):
        old_value, new_value = old_data.get(key), new_data.get(key)
        if old_value != new_value:
            if key.startswith('resolved_') or key == 'drop_rules': diffs.append(f"  - Drops changed for '{key}'")
            elif key == 'materials' and isinstance(old_value, list) and isinstance(new_value, list):
                old_mats, new_mats = {m['itemId']: m for m in old_value}, {m['itemId']: m for m in new_value}
                for mat_key in set(old_mats.keys()) | set(new_mats.keys()):
                    ...
                    if old_mat is None: diffs.append(f"  - Material Added: '{mat_name}' (Amount: {new_mat.get('amount')})")
                    elif new_mat is None: diffs.append(f"  - Material Removed: '{mat_name}'")
                    elif old_mat != new_mat:
                        if old_mat.get('amount') != new_mat.get('amount'): diffs.append(f"  - Material '{mat_name}' amount changed from {old_mat.get('amount')} to {new_mat.get('amount')}")
                        if old_mat.get('itemCategoryName_EN') != new_mat.get('itemCategoryName_EN'): diffs.append(f"  - Material '{mat_name}' category changed from '{old_mat.get('itemCategoryName_EN')}' to '{new_mat.get('itemCategoryName_EN')}'")
            else:
...
def seed_history(initial_history_path, id_key_map):
    ...
    try:
        with open(initial_history_path, 'r', encoding='utf-8') as f: resolver_data = json.load(f)
        seeded_history, initial_version = {key: {} for key in id_key_map.keys()}, "initial_seed"
...
def get_version_from_user(historical_files):
    ...
    while True:
        ...
        if not version_string: print("Version identifier cannot be empty.")
        elif version_string in existing_versions:
            if input(f"WARNING: Version '{version_string}' already exists. Overwrite? [y/N] ").lower().strip() == 'y': return version_string
            else: print("Overwrite cancelled. Please enter a different version identifier.")
        else: return version_string
...
def main():
    ...
    resolved_recipes_file, historical_recipes_file, lua_recipes_file = os.path.join(args.working_dir, 'resolved_recipes.json'), os.path.join(args.output_dir, 'historical_recipes.json'), os.path.join(args.output_dir, 'historical_recipes.lua')
    resolved_droptables_file, historical_droptables_file, lua_droptables_file = os.path.join(args.working_dir, 'resolved_droptables.json'), os.path.join(args.output_dir, 'historical_droptables.json'), os.path.join(args.output_dir, 'historical_droptables.lua')

    if not run_decryption(args.input_dir, decrypted_dir) or \
       not run_script('resolver.py', decrypted_dir, resolved_recipes_file) or \
       not run_script('droptable_resolver.py', decrypted_dir, resolved_droptables_file):
        return
    ...
    historical_recipes = json.load(open(historical_recipes_file, 'r', encoding='utf-8')) if os.path.exists(historical_recipes_file) else (seed_history(args.initial_recipe_history, recipe_id_map) if args.initial_recipe_history and os.path.exists(args.initial_recipe_history) else {"crafting": {}, "smelting": {}, "cultivation": {}})
